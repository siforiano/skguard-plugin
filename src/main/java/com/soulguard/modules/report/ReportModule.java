package com.soulguard.modules.report;
...
public class ReportModule implements SecurityModule {
    ...
    public ReportModule(SoulGuard plugin) {
        this.plugin = plugin;
    }
    ...
    public void handleReport(Player reporter, String targetName, String reason) {
        ...
        if (cooldowns.containsKey(uuid) && now - cooldowns.get(uuid) < 60000) {
            reporter.sendMessage(ColorUtil.translate("&c[SoulGuard] Please wait before sending another report."));
            return;
        }
        ...
        reporter.sendMessage(ColorUtil.translate(
                "&a[SoulGuard] Thank you! Your report against &e" + targetName + " &ahas been sent to our staff."));
    }
}

package com.soulguard.modules.system;
...
public class ConflictManager implements SecurityModule {
    ...
    public ConflictManager(SoulGuard plugin) {
        this.plugin = plugin;
    }
    ...
    public List<String> checkConflicts() {
        ...
        for (String plName : conflictingPlugins) {
            ...
            if (pl != null && pl.isEnabled()) {
                if (isAuthEnabled) {
                    conflicts.add("&c[CRITICAL] Competing authentication plugin detected: &e" + plName + "&c. Disable SoulGuard's AuthModule or the other plugin to avoid conflicts.");
                } else {
                    conflicts.add("&e[INFO] External auth plugin detected: &f" + plName + "&e. SoulGuard AuthModule is correctly disabled.");
                }
            }
        }

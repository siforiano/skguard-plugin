...
/**
 * Public API for SoulGuard Premium.
 * <p>
 * Allows third-party developers to integrate with SoulGuard,
 ...
 * <pre>{@code
 * // Get API instance
 * SoulGuardAPI api = SoulGuardAPI.getInstance();
 * 
 ...
 * @since 1.0
 */
public class SoulGuardAPI {

    private static SoulGuardAPI instance;
    ...
    private SoulGuardAPI(SoulGuard plugin) {
        this.plugin = plugin;
    }

    /**
     * Gets the SoulGuardAPI instance.
     * 
     * @return the API instance, or null if not available
     */
    public static SoulGuardAPI getInstance() {
        if (instance == null && SoulGuard.getInstance() != null) {
            instance = new SoulGuardAPI(SoulGuard.getInstance());
        }
        return instance;
    }
    ...
    public void registerCheck(Check check) {
        if (!plugin.getEdition().isPremium()) {
            throw new IllegalStateException("Custom checks require SoulGuard Premium");
        }
        
        com.soulguard.modules.anticheat.CheckManager manager = 
            plugin.getModuleManager().getModule(com.soulguard.modules.anticheat.CheckManager.class);
    ...
    public double getTrustScore(Player player) {
        try {
            com.soulguard.api.SecurityModule module = plugin.getModuleManager().getModule("Biometry");
            if (module == null) return 1.0;
            
            Object biometry = Class.forName("com.soulguard.modules.premium.BiometryModule")
    ...
    public BehaviorProfile getBehaviorProfile(Player player) {
        try {
            com.soulguard.api.SecurityModule mlModule = plugin.getModuleManager().getModule("MLDetection");
            if (mlModule != null) {
    ...
    public boolean isRecording(UUID uuid) {
        try {
            com.soulguard.api.SecurityModule forensics = plugin.getModuleManager().getModule("Forensics");
            if (forensics != null) {

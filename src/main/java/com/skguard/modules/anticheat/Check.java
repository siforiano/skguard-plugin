...
public abstract class Check {
    ...
    protected final SKGuard plugin;
    ...
    public Check(SKGuard plugin, String name, String category) {
        this.plugin = plugin;
        this.name = name;
    ...
    protected void flag(Player player, String info, double reliability) {
        ...
        String platform = com.skguard.util.BedrockManager.getPlatform(player);
        int ping = player.getPing();
        
        String alertMsg = ColorUtil.translate("&c&lSKGuard &8| &e" + errorCode + " &8| &f" + player.getName() + 
        ...
        if (plugin.getEdition().isPremium()) {
            try {
                ...
                for (Player staff : player.getServer().getOnlinePlayers()) {
                    if (staff.hasPermission("SKGuard.anticheat.alerts")) {
        ...
        if (vl >= violationsToKick) {
            player.kickPlayer(ColorUtil.translate("&c&l[SKGuard] &8(&e" + errorCode + "&8) &7Seguridad detectó actividad inusual."));
        }

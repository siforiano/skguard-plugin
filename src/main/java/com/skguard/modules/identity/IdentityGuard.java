...
public class IdentityGuard implements SecurityModule, Listener {
    ...
    public IdentityGuard(SKGuard plugin) {
        this.plugin = plugin;
    }
        ...
        if (lastKnownUuid.containsKey(name)) {
            if (!lastKnownUuid.get(name).equals(uuid)) {
                event.disallow(AsyncPlayerPreLoginEvent.Result.KICK_OTHER,
                        ColorUtil.translate(
                                "&c[SKGuard] Identity Mismatch Detected.\n&7Please use your original account."));
    ...
    public void onBookEdit(PlayerEditBookEvent event) {
        ...
        if (meta.getPages().size() > 50) {
            ...
            event.getPlayer()
                    .sendMessage(ColorUtil.translate("&c[SKGuard] Book discarded: too many pages (max 50)."));
            return;
        }
        ...
        for (String page : meta.getPages()) {
            if (page.length() > 500) {
                event.setCancelled(true);
                event.getPlayer()
                        .sendMessage(ColorUtil.translate("&c[SKGuard] Book discarded: page content too long."));
                return;
            }
        }
    }

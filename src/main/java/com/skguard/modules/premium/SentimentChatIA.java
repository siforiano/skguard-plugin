...
public class SentimentChatIA implements SecurityModule, Listener {
    ...
    public SentimentChatIA(SKGuard plugin) {
        this.plugin = plugin;
    }
    ...
    public void onChat(AsyncPlayerChatEvent event) {
        ...
        for (String pattern : toxicPatterns) {
            if (message.contains(pattern)) {
                ...
                event.getPlayer().sendMessage(ColorUtil.translate("&c&lSKGuard IA &8» &7Tu mensaje ha sido bloqueado por toxicidad."));
                return;
            }
        }
    }
    ...
    private void analyzeToxicity(Player player, String message) {
        ...
        for (Player staff : player.getServer().getOnlinePlayers()) {
            if (staff.hasPermission("SKGuard.alerts.chat")) {

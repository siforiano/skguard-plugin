...
public class DiscordBotModule implements SecurityModule {
    ...
    public DiscordBotModule(SKGuard plugin) {
        this.plugin = plugin;
        ...
        CompletableFuture.runAsync(() -> {
            try {
                this.jda = JDABuilder.createDefault(botToken, 
                        java.util.List.of(GatewayIntent.GUILD_MESSAGES, GatewayIntent.MESSAGE_CONTENT))
                        .setActivity(Activity.watching("SKGuard Security"))
                        .build()
                        .awaitReady();
    ...
    private interface DiscordCommand {
        void execute(SKGuard plugin, String[] args, Object discordUser);
    }

    private static class BanCommand implements DiscordCommand {
        @Override
        public void execute(SKGuard plugin, String[] args, Object discordUser) {
            if (args.length < 1)
                return;

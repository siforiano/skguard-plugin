...
public class PanicModule implements SecurityModule, Listener {
    ...
    public PanicModule(SKGuard plugin) {
        this.plugin = plugin;
    }
    ...
    private void kickNonStaff() {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (!player.hasPermission("SKGuard.staff") && !player.isOp()) {
    ...
    public void onCommand(PlayerCommandPreprocessEvent event) {
        if (enabled && panicActive && !event.getPlayer().hasPermission("SKGuard.admin")) {
            event.setCancelled(true);
            event.getPlayer()
                    .sendMessage(ColorUtil.translate("&c[SKGuard] Commands are disabled during Panic Mode."));
        }
    }
}

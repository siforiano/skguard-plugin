...
public class SKGuardCommand implements CommandExecutor, TabCompleter {
    ...
    public SKGuardCommand(SKGuard plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!sender.hasPermission("SKGuard.admin") && !sender.isOp()) {
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "panic" -> {
                if (plugin.getPanicModule() != null) {
                    ...
                    sender.sendMessage(ColorUtil
                            .translate("&c[SKGuard] Panic mode is now " + (newState ? "&lENABLED" : "&lDISABLED")));
                }
            }
            case "maintenance" -> {
                if (plugin.getMaintenanceModule() != null) {
                    ...
                    sender.sendMessage(ColorUtil.translate(
                            "&e[SKGuard] Maintenance mode is now " + (maintState ? "&lENABLED" : "&lDISABLED")));
                }
            }
            case "gui" -> {
                if (!(sender instanceof Player player)) {
            ...
            case "debug" -> {
                sender.sendMessage(ChatColor.GOLD + "--- SKGuard Debug ---");
                ...
                if (sender instanceof Player p) {
                    sender.sendMessage(ChatColor.YELLOW + "Your Perm (SKGuard.staff): " + ChatColor.RESET
                            + p.hasPermission("SKGuard.staff"));
            ...
            case "unban" -> handleUnpunish(sender, args, com.skguard.database.PunishmentManager.PunishmentType.BAN);
            case "mute" -> handleMute(sender, args, false);
            case "tempmute" -> handleMute(sender, args, true);
            case "unmute" -> handleUnpunish(sender, args, com.skguard.database.PunishmentManager.PunishmentType.MUTE);
    ...
    private void handleBan(CommandSender sender, String[] args, boolean temp) {
        ...
        if (temp) {
            expiry = System.currentTimeMillis() + com.skguard.util.TimeUtil.parseDuration(args[2]);
        ...
        com.skguard.database.PunishmentManager.Punishment p = new com.skguard.database.PunishmentManager.Punishment(
                target.getUniqueId(), targetIp, com.skguard.database.PunishmentManager.PunishmentType.BAN, reasonStr,
                ...
                if (target.isOnline()) {
                    ...
                    if (pOnline != null) {
                        ...
                        pOnline.kickPlayer(ColorUtil.translate(
                                "&c&l[SKGuard] &7You have been BANNED" + timeStr + "\n&fReason: &7" + reasonStr));
                    }
                }
    ...
    private void handleMute(CommandSender sender, String[] args, boolean temp) {
        ...
        com.skguard.database.PunishmentManager.Punishment p = new com.skguard.database.PunishmentManager.Punishment(
                target.getUniqueId(), targetIp, com.skguard.database.PunishmentManager.PunishmentType.MUTE, reasonStr,
                sender.getName(), System.currentTimeMillis(), finalExpiry, true);
    ...
    private void handleUnpunish(CommandSender sender, String[] args,
            com.skguard.database.PunishmentManager.PunishmentType type) {
        if (args.length < 2) {
    ...
    private void handleWarn(CommandSender sender, String[] args) {
        ...
        com.skguard.database.PunishmentManager.Punishment p = new com.skguard.database.PunishmentManager.Punishment(
                target.getUniqueId(), targetIp, com.skguard.database.PunishmentManager.PunishmentType.WARN, reasonStr,
    ...
    private void handleSession(CommandSender sender, String[] args) {
        ...
        if (args.length > 1) {
            if (!player.hasPermission("SKGuard.audit.full") && !player.isOp()) {
        ...
        String masterKey = plugin.getConfig().getString("general.master-key", "default_SKGuard_key");
        java.util.List<String> history = audit.getDecryptedHistory(target.getUniqueId(), masterKey);

        player.sendMessage(ColorUtil.translate("&6&l[SKGuard] &7Session History for &f" + target.getName()));
        if (history.isEmpty()) {
    ...
    private void sendInfo(CommandSender sender) {
        ...
        sender.sendMessage(ChatColor.DARK_AQUA + "█▀▀ " + ChatColor.AQUA + "" + ChatColor.BOLD + "SKGuard Security"
    ...
    private void sendHelp(CommandSender sender) {
        sender.sendMessage(ChatColor.GOLD + "--- SKGuard Help ---");
